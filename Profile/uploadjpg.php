...
<?php
    //Konfiguracja
    $maxFileSize = 2621440; //maksymalna wielko&para;ć pliku w bajtach
    $uploadFolder = 'C:/xampp/htdocs/MojProjekt/Files/'.$UserID; 
    $acceptMIME = array('image/gif', 'image/jpeg', 'image/pjpeg'); 
    ...
    if(@$_GET['a'] == 'upload')
    	...
    	if(empty($picture['error']))
    		...
    		$sql="SELECT name FROM JPGFILES WHERE name='".$uploadFolder.'/'.$picture['name']."'";
    			...
    			if(!$send_picture) $errorMsg = 'Wystapil problem podczas kopiowania pliku do wyznaczonego folderu!';
    			else {
                           
                             $successMsg = 'Kopiowanie pliku zakonczone sukcesem!';
                             $note=$mysqli->escape_string($_POST['note']);
                             $sql="insert into jpgfiles (id_User,size,name,type,note) values(".$UserID.",'".$picture['size']."','".$uploadFolder.'/'.$picture['name']."','".$picture['type']."','$note')";
                    $mysqli->query($sql);
...
<form class="form3" enctype="multipart/form-data"  name="posting" action="uploadjpg.php?a=upload" method="post">
    <h2>Dodaj plik .jpg</h2>
  <table class="table table-hover">
   ...
   <div class="input-file-container">  
    <input class="input-file" id="upload_picture" type="file" name="upload_picture">
...
<br>
<button class="button button-block" name="picture_send"/>Wyślij</button>
 ...
 <?php if(!empty($successMsg)) echo '<div class="info2">'.$successMsg.'</div>'; ?>
    ...
    <?php
     ...
     $q="SELECT count(*)as howMany FROM jpgfiles WHERE id_user='$UserID';";
    $r=$mysqli->query($q);
    $many=$r->fetch_assoc();
    if($many['howMany']!=0)
    {
...
echo "<tbody>";
    ...
    closedir($dir);
    @sort($read_file);
    $r=$mysqli->query("Select * from jpgfiles where id_User=$UserID;");
...
while($row=$r->fetch_assoc()){
    echo "<tr><td>".str_replace("C:/xampp/htdocs/MojProjekt/Files/$UserID/", "", $row['name'])."</td><td>".$row['note']."</td><td><a href=detailsjpg.php?id_jpgfile=".$row['id_jpgfile'].">Szczegoły</a></td></tr>";
}
